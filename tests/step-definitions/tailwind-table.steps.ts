...
import { verifyColumnHeaders, verifyTableData, verifyActionButtons } from '../utils/table-testing/verifiers';
...
const getStoryData = (storyId: string) => {
  ...
  const story = TailwindTableStories[storyId as keyof typeof TailwindTableStories];
...
// Viewing table data
When('I view the {string} {string}', async ({ page }: World, tableName: string, roleStr: string) => {
  ...
  // @ts-expect-error we need to work out the differences with rtl
  await page.getByRole(roleStr, { name: tableName }).waitFor({ state: 'visible' });

  // Get story data directly from imported stories
  const { schema, formData } = getStoryData(storyId);

  // Store context data in the page's storage state
  await page.evaluate((contextData) => {
...
const getTableContext = async (page: Page): Promise<TableContext> => {
  ...
  if (!contextData) {
    throw new Error('Table test context not found - ensure you run the previous steps');
  }
...
Then('I should see column headers based on the schema properties', async ({ page }: World) => {
  const { tableTester, tableName, tableRole, schema } = await getTableContext(page);
...
Then('I should see rows displaying my data', async ({ page }: World) => {
  const { tableTester, tableName, tableRole, schema, formData } = await getTableContext(page);
